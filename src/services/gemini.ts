...
const getApiKey = () => {
  // Em ambiente Vite/AI Studio, usamos process.env.GEMINI_API_KEY que é injetado via vite.config.ts
...
const callWithRetry = async (fn: () => Promise<any>, maxRetries = 3) => {
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      // Se for erro de quota (429), tenta novamente com backoff exponencial
      ...
      // Se for erro de segurança ou outros erros fatais, não retenta
      throw error;
    }
  }
  
  if (lastError?.message?.includes("429") || JSON.stringify(lastError).includes("429") || lastError?.status === "RESOURCE_EXHAUSTED") {
    throw new Error("O limite de uso gratuito foi atingido. Por favor, aguarde um momento e tente novamente.");
  }
  throw lastError;
};
...
export const generateMessage = async (prompt: string) => {
  ...
  if (!apiKey) {
    throw new Error("Chave de API não configurada. Adicione GEMINI_API_KEY aos Secrets.");
  }
  ...
  return await callWithRetry(async () => {
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction: "Você é um assistente carinhoso e poético. Crie mensagens de Dia das Mães em português do Brasil que sejam tocantes e originais. Use emojis de forma moderada e elegante.",
